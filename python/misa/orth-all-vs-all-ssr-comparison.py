    ...
    for i in range(len(cluster)):
      ...
      for j in range(i+1,len(cluster)):
        ...
        if len(q_ssrs) == 0 and len(o_ssrs) == 0: 
          break
        if len(q_ssrs) == 0:
          loss[distances[qspecies + "," + ospecies]] += len(o_ssrs)
          break
        if len(o_ssrs) == 0:
          loss[distances[qspecies + "," + ospecies]] += len(q_ssrs)
          break
...
#          print m1.to_s()
#          print m2.to_s()
              perfect[distances[qspecies + "," + ospecies]] += 1
...
#          print m1.to_s()
#          print m2.to_s()
              poly[distances[qspecies + "," + ospecies]] += 1
              caught[hash(m1.to_s())] = 1
              caught[hash(m2.to_s())] = 1
...
#          print m1.to_s()
#          print m2.to_s()
              shift[distances[qspecies + "," + ospecies]] += 1
              caught[hash(m1.to_s())] = 1
              caught[hash(m2.to_s())] = 1
...
#      print spec2 + "\t" + m2.to_s()
        loss[distances[qspecies + "," + ospecies]] += len(q_ssrs) + len(o_ssrs) - mapped 


  keys = perfect.keys()
  keys.sort()
  for time in keys:
    perfectcount = str(perfect[time])
    polycount = str(poly[time])
    shiftcount = str(shift[time])
    losscount = str(loss[time])
    totalcount = str(perfectcount + polycount + shiftcount + losscount)
    print string.join([time, perfectcount, polycount, shiftcount, losscount, totalcount], "\t")
